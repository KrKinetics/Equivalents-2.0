...
test('intake actions remain available for every service group', () => {
  ...
  const motivationIdx = dashboardJs.indexOf('data-group="motivation"');
  const manageIdx = dashboardJs.indexOf('client-management-actions');
  assert.ok(intakeIdx > 0 && intakeIdx < nutritionIdx && nutritionIdx < motivationIdx && motivationIdx < manageIdx);
