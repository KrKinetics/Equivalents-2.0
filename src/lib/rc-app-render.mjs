/**
 * Interactive release-candidate HTML (KR Kinetics technical identity).
 * Visual language aligned with the existing professional calculator
 * (Segoe UI, slate/blue palette, compact tool layout).
 * Browser calculator uses the same data bundle produced by rc-preview.
 */

export function buildReleaseCandidateHtml({
  dataUrl = './rc-data.json',
...
<title>KR Kinetics — Calculateur · version candidate</title>
...
.provisional-banner {
  ...
  text-align: center; padding: 0.5rem 1rem; font-weight: 700; font-size: 0.86rem;
}
.wrap { width: min(1300px, calc(100% - 40px)); margin: 0 auto; padding: 20px 0 2.5rem; }
.app-header {
  display: flex; align-items: center; justify-content: space-between; gap: 1.25rem;
  background: var(--navy); color: #fff; border-radius: 10px;
  padding: 18px 28px; margin-bottom: 25px; box-shadow: var(--shadow);
}
.header-logo {
...
.header-logo img {
  max-height: 72px; width: auto; max-width: min(340px, 48vw);
  object-fit: contain; display: block;
  background: transparent; filter: none; box-shadow: none;
}
.header-title-container { flex: 1; text-align: center; min-width: 0; padding: 0 12px; }
.header-title-container h1 {
  color: #fff; margin: 0 0 5px; padding: 0; border: none;
  font-size: 1.55rem; letter-spacing: 1px; font-weight: 700;
}
.header-title-container p { margin: 0; color: #cbd5e1; font-size: 0.9rem; }
.panel {
  background-color: var(--card-bg); border: 1px solid var(--border); border-radius: 10px;
  box-shadow: var(--shadow); padding: 25px; margin-bottom: 25px; max-width: 100%;
}
.panel h2 {
  color: var(--navy); margin: 0 0 15px; padding-bottom: 10px;
  border-bottom: 2px solid var(--border); font-size: 1.3rem; font-weight: 700;
}
.mode-bar { display: grid; gap: 0.75rem; }
@media (min-width: 768px) { .mode-bar { grid-template-columns: 1fr 1fr; } }
.mode-option {
  border: 2px solid var(--border); border-radius: 8px; padding: 0.8rem 0.95rem;
  cursor: pointer; background: var(--bg); transition: border-color .15s, background .15s, box-shadow .15s;
}
.mode-option:hover { border-color: #fca5a5; }
.mode-option.active {
  border-color: var(--brand-red); background: #fff5f6;
  box-shadow: 0 0 0 3px rgba(217,17,54,0.15);
}
.mode-option strong { display: block; margin-bottom: 0.2rem; color: var(--navy); }
.mode-option span { color: var(--muted); font-size: 0.9rem; }
.grid-2 { display: grid; gap: 1rem; }
@media (min-width: 900px) { .grid-2 { grid-template-columns: 1fr 1fr; } }
.totals { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 0.55rem; }
@media (min-width: 768px) { .totals { grid-template-columns: repeat(4, minmax(0, 1fr)); } }
.stat {
  background: #f1f5f9; border: 1px solid var(--border); border-left: 4px solid var(--navy);
  border-radius: 8px; padding: 0.65rem 0.7rem; min-width: 0;
}
.stat .label {
  color: var(--muted); font-size: 0.72rem; text-transform: uppercase;
  font-weight: 700; letter-spacing: 0.04em;
}
.stat .value { font-size: 1.2rem; font-weight: 800; margin-top: 0.15rem; color: var(--navy); overflow-wrap: anywhere; }
.stat .value.na { font-size: 0.88rem; font-weight: 600; color: var(--muted); }
.groups { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 0.55rem; }
@media (min-width: 768px) { .groups { grid-template-columns: repeat(4, minmax(0, 1fr)); } }
.group-card label { display: block; font-size: 0.82rem; font-weight: 600; color: var(--secondary); margin-bottom: 0.25rem; }
.group-card input, input[type="search"], select, .qty-input {
  width: 100%; border: 1px solid var(--border); border-radius: 6px;
  padding: 0.5rem 0.65rem; font: inherit; background: #fcfcfc; color: var(--secondary);
}
.group-card input:focus, input:focus, select:focus {
  outline: none; border-color: var(--brand-red); box-shadow: 0 0 0 3px rgba(217,17,54,0.15);
}
button, .btn {
  appearance: none; border: none; border-radius: 7px; background: var(--brand-red); color: #fff;
  font: inherit; font-weight: 700; padding: 0.6rem 0.9rem; cursor: pointer; letter-spacing: 0.2px;
}
button:hover, .btn:hover { background: #b40e2c; }
...
.actions { display: flex; flex-wrap: wrap; gap: 0.55rem; margin-top: 0.85rem; align-items: center; }
.warn-list { margin: 0.5rem 0 0; padding-left: 1.1rem; color: #991b1b; }
.compare-table { width: 100%; border-collapse: collapse; font-size: 0.86rem; table-layout: fixed; }
.compare-table th, .compare-table td {
  border: 1px solid var(--border); text-align: left; padding: 0.45rem 0.4rem;
  vertical-align: top; word-break: break-word; overflow-wrap: anywhere;
}
.compare-table th { background: #f1f5f9; color: var(--navy); font-weight: 700; font-size: 0.8rem; }
.filters { display: grid; gap: 0.55rem; }
@media (min-width: 900px) { .filters { grid-template-columns: 1.4fr 1fr 1fr 1fr; } }
select { max-width: 100%; text-overflow: ellipsis; }
.food-list {
  border: 1px solid var(--border); border-radius: 8px; margin-top: 0.5rem;
  max-height: min(520px, 60vh); overflow: auto;
}
.food-row {
  display: grid; gap: 0.35rem; padding: 0.55rem 0.75rem; border-bottom: 1px solid var(--border);
  grid-template-columns: 1fr auto; align-items: center;
}
.food-row:nth-child(even) { background: #fafbfc; }
...
details.diagnostics { margin-top: 0.15rem; }
details.diagnostics > summary {
  cursor: pointer; font-weight: 700; color: var(--secondary); list-style: none; padding: 0.3rem 0;
}
...
.scenario {
  ...
  margin-bottom: 0.5rem; background: var(--ok-bg);
}
.pill {
...
.footer-note { color: var(--muted); font-size: 0.84rem; margin-top: 1.25rem; }
.hidden { display: none !important; }
.food-pager { display: flex; flex-wrap: wrap; gap: 0.55rem; align-items: center; margin-top: 0.65rem; }
@media (max-width: 768px) {
  .app-header { flex-direction: column; gap: 12px; text-align: center; padding: 16px 18px; }
  .header-logo img { max-height: 56px; max-width: min(280px, 80vw); }
...
<div class="wrap">
  <header class="app-header">
    <div class="header-logo">
      <img id="brand-logo" src="${logoUrl}" alt="KR Kinetics" />
    </div>
    <div class="header-title-container">
      <h1>Calculateur d’équivalents alimentaires</h1>
      <p>Outil principal — calcul actuel par défaut · aperçu personnalisé non approuvé</p>
    </div>
  </header>
  ...
  <section class="panel">
    ...
    <div class="food-pager">
      <button type="button" class="secondary" id="btn-food-more">Afficher plus</button>
      <button type="button" class="secondary" id="btn-food-all">Afficher tout</button>
    </div>
  </section>
...
let cart = [];
let foodVisibleLimit = FOOD_PAGE_SIZE;
let filteredFoodRows = [];
...
function renderFoods({ resetLimit = false } = {}) {
  if (resetLimit) foodVisibleLimit = FOOD_PAGE_SIZE;
  filteredFoodRows = filterFoodRows();
  const totalCatalog = DATA.foods.length;
  const filtered = filteredFoodRows.length;
  const shown = Math.min(foodVisibleLimit, filtered);
  document.getElementById('food-count').textContent =
    shown + ' / ' + filtered + ' aliments affichés · catalogue exact : ' + totalCatalog;
  document.getElementById('food-list').innerHTML = filteredFoodRows.slice(0, shown).map((food) => {
    const a = DATA.assignmentsByFoodId[food.id];
    return '<div class="food-row">'+
      '<div><strong>'+(food.names?.fr || food.id)+'</strong><div class="muted">'+(food.names?.en || '')+' · '+(GROUP_LABELS[food.calculationGroup]||food.calculationGroup)+(a?.exchangeRollupId ? ' · '+a.exchangeRollupId.replace(/^rollup-/, '') : '')+'</div></div>'+
      '<button type="button" class="secondary" data-add="'+food.id+'">+1</button>'+
    '</div>';
  }).join('') || '<div class="food-row"><div class="muted">Aucun aliment pour ces filtres.</div></div>';
  document.querySelectorAll('[data-add]').forEach((btn) => {
    btn.addEventListener('click', () => addToCart(btn.getAttribute('data-add')));
  });
  const moreBtn = document.getElementById('btn-food-more');
  const allBtn = document.getElementById('btn-food-all');
  const canShowMore = shown < filtered;
  moreBtn.disabled = !canShowMore;
  allBtn.disabled = !canShowMore;
  moreBtn.classList.toggle('hidden', filtered === 0);
  allBtn.classList.toggle('hidden', filtered === 0);
}
...
function renderEmptyComparison() {
  document.getElementById('compare-totals').innerHTML = [
    ...
    return '<div class="stat"><div class="label">'+label+'</div><div class="value na">'+(value ?? '—')+'</div></div>';
  }).join('');
  document.getElementById('diff-summary').textContent =
    'Panier vide : aucune comparaison A contre aperçu (pas d’écarts fictifs).';
  document.querySelector('#lines-table tbody').innerHTML =
    '<tr><td colspan="4" class="muted">Ajoutez des aliments au panier pour comparer ligne à ligne.</td></tr>';
}
...
function render() {
  ...
  const legacyCart = cart.length ? calcLegacyFromCartFoods() : null;
  ...
  document.getElementById('validation-details').textContent = JSON.stringify({
    ...
    foodCatalogCount: DATA.foods.length,
    foodVisibleLimit,
  }, null, 2);
}
...
async function boot() {
  ...
  document.getElementById('btn-food-more').addEventListener('click', () => {
    foodVisibleLimit = Math.min(filteredFoodRows.length, foodVisibleLimit + FOOD_PAGE_SIZE);
    renderFoods();
  });
  document.getElementById('btn-food-all').addEventListener('click', () => {
    foodVisibleLimit = filteredFoodRows.length;
    renderFoods();
  });
  ['food-search','filter-category','filter-group','filter-rollup'].forEach((id) => {
    document.getElementById(id).addEventListener('input', () => renderFoods({ resetLimit: true }));
    document.getElementById(id).addEventListener('change', () => renderFoods({ resetLimit: true }));
  });
}
boot().catch((err) => {
  document.body.insertAdjacentHTML('beforeend', '<pre style="color:#991b1b;padding:1rem;">Erreur de chargement : '+String(err)+'</pre>');
});
</script>
</body>
</html>`;
