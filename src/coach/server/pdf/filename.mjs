import { getPdfTheme, normalizeBrandToken } from './themes.mjs';
...
export function buildPdfFilename({ locale = 'fr', brandSlug, athleteName, dateIso } = {}) {
  const brandId = normalizeBrandToken(brandSlug)
    || (brandSlug === 'kr' || brandSlug === 'elevate' ? brandSlug : 'kr');
  const brand = getPdfTheme(brandId).filenameSlug || BRAND_FILENAMES[brandId] || BRAND_FILENAMES.kr;
